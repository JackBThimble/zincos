...
fn ensureTable(ctx: *X64MapperCtx, entry: *volatile u64) u64 {
...
pub const MapperCtx = X64MapperCtx;

pub const X64MapperCtx = struct {
    hhdm_base: u64,
    fa: *pmm.FrameAllocator,

    /// Create a vmm.Mapper interface from this context
    pub fn mapper(self: *X64MapperCtx) vmm.Mapper {
        return vmm.Mapper{
            .ptr = self,
            .vtable = &vtable,
        };
    ...
    fn map4kImpl(ptr: *anyopaque, virt: u64, phys_addr: u64, flags: vmm.MapFlags) void {
        const self: *X64MapperCtx = @ptrCast(@alignCast(ptr));
        std.debug.assert(vmm.isAligned(virt));
        std.debug.assert(vmm.isAligned(phys_addr));
    ...
    fn pteSlot(self: *X64MapperCtx, virt: u64) ?*volatile u64 {
        const pml4_phys = readCr3() & ~@as(u64, 0xfff);
        const pml4 = tableVirt(self.hhdm_base, pml4_phys);
